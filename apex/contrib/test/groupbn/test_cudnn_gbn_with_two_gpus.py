...
try:
    ...
    from apex.contrib.groupbn.batch_norm import BatchNorm2d_NHWC as GBN_old
    ...
    def __init__(self, num_features, num_layers=1000):
        ...
        self.fwd = nn.Sequential(
            *[
                ...
                GBN_old(num_features, fuse_relu=False, bn_group=2, max_cta_per_sm=2, cta_launch_margin=12, multi_stream=False)
                for _ in range(num_layers)
            ]
        )
    ...
    def _test_cudnn_gbn(
        ...
        *,
        ...
        with torch.no_grad():
            ...
            input = input[self.rank : self.rank + 1, ...].half().clone().detach().requires_grad_()
